...
// TEMPORARY FIX: Comment out requireAuth to allow guest cart
...
customerCartWishlistRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      // Try to get user if signed in, otherwise return empty cart
      const dbUser = await getDbUserFromReq(req);
      res.json(ok(await getCartResponse(String(dbUser._id))));
    } catch {
      // User not signed in, return empty cart
...
customerCartWishlistRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      // User not signed in or other error - guest should use localStorage
      if (error instanceof AppError) {
        throw error;
      }
      throw new AppError(401, "Please sign in to add items to cart");
    }
  }),
);
