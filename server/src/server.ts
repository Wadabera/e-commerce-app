...
async function mainEntryFunction() {
  ...
  // Apply Clerk middleware FIRST (before any routes)
  if (process.env.CLERK_PUBLISHABLE_KEY && process.env.CLERK_SECRET_KEY) {
    app.use(clerkMiddleware());
    console.log("✅ Clerk middleware applied globally");
  } else {
    console.warn("⚠️  Clerk keys are missing; protected customer routes are disabled");
  }
  ...
  // customer routes (Clerk middleware already applied globally above)
  ...
  // admin routes (NO Clerk middleware! Uses requireAdminToken instead)
